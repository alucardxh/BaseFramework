...
public class DateUtils {
	 ...
	 * @
	 */
	public static int getWeekOfDate(Date datetime) throws ParseException {
		Calendar cal = Calendar.getInstance();
		cal.setFirstDayOfWeek(Calendar.MONDAY);
		cal.setMinimalDaysInFirstWeek(WEEK_DAYS);
	 ...
	 * @throws ParseException
	 */
	public static String getCalculateDateToString(Date date, int time) {
		Calendar c = Calendar.getInstance();
		c.setTime(date);
		c.set(Calendar.DAY_OF_YEAR, time);
		date = c.getTime();
		return formatDateToString(date, DATE_FORMAT_YMD);
	}
	 ...
	 * @
	 */
	public static int[] getYearAndWeeks(String date, String admitDate) throws ParseException {
		Calendar c = Calendar.getInstance();
	...
	public static String getMonthOfDate(Date date) {
		SimpleDateFormat dateFm = new SimpleDateFormat("M");
		return dateFm.format(date);
	}

	public static String getCurrentOfDate(Date date) {
		SimpleDateFormat dateFm = new SimpleDateFormat("d");
		return dateFm.format(date);
	}
	...
	public static List<String> getMonth(int year, int month) throws Exception {
		...
		if (week - 1 > 0) {
			if (month == 1) {
				List<String> lastMonthDays = getLastMonthDay(year - 1, 12, week - 1);
				list.addAll(lastMonthDays);
			} else {
				List<String> lastMonthDays = getLastMonthDay(year, month - 1, week - 1);
				list.addAll(lastMonthDays);
			}
		}
		List<String> list2 = getMonthDay(year, month);
		list.addAll(list2);
		int nextDay = 36 - list.size();
	 ...
	 * @throws ParseException
	 */
	private static int getMonthFirstDayWeek(int year, int month) throws ParseException {
		Calendar cal = Calendar.getInstance();
		cal.set(Calendar.YEAR, year);
		...
		int week = getWeekOfDate(firstDate);
		return week;
	}
	 ...
	 * @throws Exception
	 */
	private static List<String> getLastMonthDay(int year, int month, int amountDay) throws Exception {
		Calendar cal = Calendar.getInstance();
		cal.set(Calendar.YEAR, year);
		cal.set(Calendar.MONTH, month);
		cal.set(Calendar.DAY_OF_MONTH, 1);
		...
		String lastDayStr = getCurrentOfDate(lastDate);
		int lastDay = Integer.valueOf(lastDayStr);
		int startDay = lastDay - amountDay + 1;
		List<String> list = new ArrayList<String>();
		for (int i = 0; i < amountDay; i++) {
			list.add(String.valueOf(startDay++));
		}
		return list;
	}
	 ...
	 * @throws ParseException
	 */
	private static String getMonthFinalDay(int year, int month) throws ParseException {
		Calendar cal = Calendar.getInstance();
		cal.set(Calendar.YEAR, year);
		cal.set(Calendar.MONTH, month);
		cal.set(Calendar.DAY_OF_MONTH, 1);
	 ...
	 * @throws ParseException
	 */
	private static List<String> getMonthDay(int year, int month) throws ParseException {
		List<String> list = new ArrayList<String>();
		String lastDay = getMonthFinalDay(year, month);
		for (int i = 1; i < Integer.valueOf(lastDay) + 1; i++) {
			list.add(String.valueOf(i));
		}
		return list;
	}
